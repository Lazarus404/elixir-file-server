defmodule MoveTest do
  ...
  test "move uploaded asset", %{token: token} do
    ...
    conn = move_file(token, "#{@type_asset}/#{@asset}", %{
  ...
  test "failing unauthorised moving of uploaded asset", %{token: token, user_token: user_token} do
    upload_file(token, @type_asset, @example_file)
    ...
    conn = move_file(@bad_token, "#{@type_asset}/#{@asset}", %{
      "target" => "#{@type_asset}/#{@new_path}"
    })
    ...
    conn = move_file(user_token, "#{@type_asset}/#{@asset}", %{
      "target" => "#{@type_asset}/#{@new_path}"
    })
  ...
  test "move uploaded file", %{token: token} do
    ...
    conn = move_file(token, "#{@type_file}/#{@path}/#{@asset}", %{
  ...
  test "failing unauthorised moving of uploaded file", %{token: token, user_token: user_token} do
    upload_file(token, Path.join(@type_file, @path), @example_file)
    ...
    conn = move_file(@bad_token, "#{@type_file}/#{@path}/#{@asset}", %{
      "target" => "#{@type_file}/#{@new_path}"
    })
    ...
    conn = move_file(user_token, "#{@type_file}/#{@path}/#{@asset}", %{
      "target" => "#{@type_file}/#{@new_path}"
    })
  ...
  test "move uploaded asset to file directory", %{token: token} do
    upload_file(token, @type_asset, @example_file)
  ...
  test "move multiple uploaded assets", %{token: token} do
    upload_file(token, @type_asset, @example_file)
  ...
  test "failing unauthorised moving of multiple uploaded assets", %{token: token, user_token: user_token} do
    upload_file(token, @type_asset, @example_file)
    upload_file(token, @type_asset, @example_file_other)
  ...
  defp move_file(token, path, params \\ %{}) do
    call_with_token(token, :post, "move/#{path}", params)
  end
